...
export class ValidationFormService {
  ...
  getDefaultValues(idOrg?: number, regne?: string, group2_inpn?: string) {
    ...
    return this._http.get<any>(
      `${AppConfig.API_ENDPOINT}/validation/defaultNomenclatures`,
      {
        params: params
      }
    );
  ...
  initReleveForm(): FormGroup {
    const releveForm = this._fb.group({
      ...
      properties: this._fb.group({
        ...
        observers: [
          null,
          !ValidationConfig.form_fields.releve.observers_txt
            ? Validators.required
            : null
        ],
        observers_txt: [
        ...
        t_occurrences_validation: [new Array()]
      })
    });
  ...
  addOccurrence(index, cancel?: boolean) {
    ...
    // push the counting
    this.occurrenceForm.controls.cor_counting_validation.patchValue(
    ...
    if (
      this.releveForm.value.properties.t_occurrences_validation.length ===
      this.indexOccurrence
    ) {
      // push the current taxon in the taxon list
      this.taxonsList.push(this.currentTaxon);
      this.releveForm.value.properties.t_occurrences_validation.push(
        this.occurrenceForm.value
      );
    } else {
      if (cancel) {
        // push the saved occurrence
        this.releveForm.value.properties.t_occurrences_validation[
          this.indexOccurrence
    ...
    this.indexOccurrence = this.releveForm.value.properties.t_occurrences_validation.length;
    // reset counting
    this.nbCounting = [""];
    this.indexCounting = 0;
    // reset current taxon
  ...
  editOccurence(index) {
    ...
    const occurenceData = this.releveForm.value.properties
      .t_occurrences_validation[index];
    this.savedOccurrenceData = Object.assign(
      {},
      this.releveForm.value.properties.t_occurrences_validation[index]
    );

    const countingData = occurenceData.cor_counting_validation;
  ...
  removeOneOccurrence(index) {
    this.taxonsList.splice(index, 1);
    this.releveForm.value.properties.t_occurrences_validation.splice(index, 1);
  ...
  onEditReleve(id) {
    this._router.navigate(["validation/form", id]);
  }
  backToList() {
