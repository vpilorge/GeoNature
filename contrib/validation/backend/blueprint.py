...
@blueprint.route('/vrelevevalidation', methods=['GET'])
@fnauth.check_auth_cruved('R', True)
@json_resp
...
def getViewReleveList(info_role):
    ...
    if 'cd_nom' in params:
        ...
        q = q.join(
            TOccurrencesContact,
            TOccurrencesContact.id_releve_validation ==
            VReleveList.id_releve_validation
        ).filter(
    ...
    if 'observer' in params:
        q = q.join(
            ...
            corRoleRelevesContact.columns.id_releve_validation ==
            VReleveList.id_releve_validation
        ).filter(
    ...
    if len(releve_filters) > 0:
        q = q.join(
            TRelevesContact,
            VReleveList.id_releve_validation ==
            TRelevesContact.id_releve_validation
        )
    ...
    if len(counting_filters) > 0:
        if len(occurrence_filters) > 0:
            q = q.join(
                CorCountingContact,
                TOccurrencesContact.id_occurrence_validation ==
                CorCountingContact.id_occurrence_validation
            )
        else:
            q = q.join(
                TOccurrencesContact,
...
def insertOrUpdateOneReleve(info_role):
    ...
    if 't_occurrences_validation' in data['properties']:
        occurrences_validation = data['properties']['t_occurrences_validation']
        data['properties'].pop('t_occurrences_validation')
    ...
    for occ in occurrences_validation:
        if occ['cor_counting_validation']:
            cor_counting_validation = occ['cor_counting_validation']
            occ.pop('cor_counting_validation')
        ...
        validation = TOccurrencesContact(**occ)
        for cnt in cor_counting_validation:
            # Test et suppression
            ...
            validation.cor_counting_validation.append(countingContact)
        releve.t_occurrences_validation.append(validation)

    if releve.id_releve_validation:
...
def deleteOneReleve(id_releve, info_role):
    """Suppression d'une données d'un relevé et des occurences associés
      c-a-d un enregistrement de la table t_releves_validation
...
def deleteOneOccurence(id_occ):
    """Suppression d'une données d'occurrence et des dénombrements associés
      c-a-d un enregistrement de la table t_occurrences_validation
...
def deleteOneOccurenceCounting(id_count):
    """Suppression d'une données de dénombrement
      c-a-d un enregistrement de la table cor_counting_validation
...
def getDefaultNomenclatures():
    ...
    q = DB.session.query(
        ...
        func.pr_validation.get_default_nomenclature_value(
...
def export_sinp(info_role):
    """ Return the data (CSV) at SINP format
        from pr_validation.export_occtax_sinp view
    If no paramater return all the dataset allowed of the user
    params:
        - id_dataset : integer
        - uuid_dataset: uuid
    """
    viewSINP = GenericTable('export_occtax_dlb', 'pr_validation', None)
