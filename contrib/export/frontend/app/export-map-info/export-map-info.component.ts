...
@Component({
  selector: "pnx-export-map-info",
  templateUrl: "export-map-info.component.html",
  styleUrls: ["./export-map-info.component.scss"]
})
export class ExportMapInfoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._sub = this._route.params.subscribe(params => {
      ...
      if (!isNaN(this.id)) {
        // load one releve
        this._exportService.getOneReleve(this.id).subscribe(data => {
          this.userReleveCruved = data.cruved;

          this.releveForm.patchValue(data.releve);
          this.releve = data.releve;
          if (!ExportConfig.form_fields.releve.observers_txt) {
          ...
          // load taxonomy info
          data.releve.properties.t_occurrences_export.forEach(occ => {
