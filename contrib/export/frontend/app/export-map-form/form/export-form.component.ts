...
export class ExportFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!isNaN(this.id)) {
      ...
      // load one releve
      this.exportService.getOneReleve(this.id).subscribe(data => {
        // pre fill the form
        ...
        // set the occurrence
        this.fs.indexOccurrence = data.releve.properties.t_occurrences_export.length;
  ...
  submitData() {
    ...
    finalForm.properties.t_occurrences_export.forEach((occ, index) => {
    ...
    this._cfs.postExport(finalForm).subscribe(
      response => {
