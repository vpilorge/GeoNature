...
export class ExportFormService {
  ...
  getDefaultValues(idOrg?: number, regne?: string, group2_inpn?: string) {
    ...
    return this._http.get<any>(
      `${AppConfig.API_ENDPOINT}/export/defaultNomenclatures`,
      {
        params: params
      }
    );
  ...
  initReleveForm(): FormGroup {
    const releveForm = this._fb.group({
      ...
      properties: this._fb.group({
        ...
        observers: [
          null,
          !ExportConfig.form_fields.releve.observers_txt
            ? Validators.required
            : null
        ],
        observers_txt: [
        ...
        t_occurrences_export: [new Array()]
      })
    });
  ...
  addOccurrence(index, cancel?: boolean) {
    ...
    // push the counting
    this.occurrenceForm.controls.cor_counting_export.patchValue(
    ...
    if (
      this.releveForm.value.properties.t_occurrences_export.length ===
      this.indexOccurrence
    ) {
      // push the current taxon in the taxon list
      this.taxonsList.push(this.currentTaxon);
      this.releveForm.value.properties.t_occurrences_export.push(
        this.occurrenceForm.value
      );
    } else {
      if (cancel) {
        // push the saved occurrence
        this.releveForm.value.properties.t_occurrences_export[
          this.indexOccurrence
    ...
    this.indexOccurrence = this.releveForm.value.properties.t_occurrences_export.length;
    // reset counting
    this.nbCounting = [""];
    this.indexCounting = 0;
    // reset current taxon
  ...
  editOccurence(index) {
    ...
    const occurenceData = this.releveForm.value.properties
      .t_occurrences_export[index];
    this.savedOccurrenceData = Object.assign(
      {},
      this.releveForm.value.properties.t_occurrences_export[index]
    );

    const countingData = occurenceData.cor_counting_export;
  ...
  removeOneOccurrence(index) {
    this.taxonsList.splice(index, 1);
    this.releveForm.value.properties.t_occurrences_export.splice(index, 1);
  ...
  onEditReleve(id) {
    this._router.navigate(["export/form", id]);
  }
  backToList() {
