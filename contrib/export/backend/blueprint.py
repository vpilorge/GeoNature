...
@blueprint.route('/vreleveexport', methods=['GET'])
@fnauth.check_auth_cruved('R', True)
@json_resp
...
def getViewReleveList(info_role):
    ...
    if 'cd_nom' in params:
        ...
        q = q.join(
            TOccurrencesContact,
            TOccurrencesContact.id_releve_export ==
            VReleveList.id_releve_export
        ).filter(
    ...
    if 'observer' in params:
        q = q.join(
            ...
            corRoleRelevesContact.columns.id_releve_export ==
            VReleveList.id_releve_export
        ).filter(
    ...
    if len(releve_filters) > 0:
        q = q.join(
            TRelevesContact,
            VReleveList.id_releve_export ==
            TRelevesContact.id_releve_export
        )
    ...
    if len(counting_filters) > 0:
        if len(occurrence_filters) > 0:
            q = q.join(
                CorCountingContact,
                TOccurrencesContact.id_occurrence_export ==
                CorCountingContact.id_occurrence_export
            )
        else:
            q = q.join(
                TOccurrencesContact,
...
def insertOrUpdateOneReleve(info_role):
    ...
    if 't_occurrences_export' in data['properties']:
        occurrences_export = data['properties']['t_occurrences_export']
        data['properties'].pop('t_occurrences_export')
    ...
    for occ in occurrences_export:
        if occ['cor_counting_export']:
            cor_counting_export = occ['cor_counting_export']
            occ.pop('cor_counting_export')
        ...
        export = TOccurrencesContact(**occ)
        for cnt in cor_counting_export:
            # Test et suppression
            ...
            export.cor_counting_export.append(countingContact)
        releve.t_occurrences_export.append(export)

    if releve.id_releve_export:
...
def deleteOneReleve(id_releve, info_role):
    """Suppression d'une données d'un relevé et des occurences associés
      c-a-d un enregistrement de la table t_releves_export
...
def deleteOneOccurence(id_occ):
    """Suppression d'une données d'occurrence et des dénombrements associés
      c-a-d un enregistrement de la table t_occurrences_export
...
def deleteOneOccurenceCounting(id_count):
    """Suppression d'une données de dénombrement
      c-a-d un enregistrement de la table cor_counting_export
...
def getDefaultNomenclatures():
    ...
    q = DB.session.query(
        ...
        func.pr_export.get_default_nomenclature_value(
...
def export_sinp(info_role):
    """ Return the data (CSV) at SINP format
        from pr_export.export_occtax_sinp view
    If no paramater return all the dataset allowed of the user
    params:
        - id_dataset : integer
        - uuid_dataset: uuid
    """
    viewSINP = GenericTable('export_occtax_dlb', 'pr_export', None)
